class Polynomial(object):
    ...
    def division(self, other):
        ...
        if (other.degree() == 0) and (other.coefficients[0] == 0):
            return  "Divide by zero polynomial"
        if (self.degree() < other.degree()):
            return Polynomial(list)
        exponent_max = max(self.degree(), other.degree())
        coefficient = self.coefficients[self.degree()]/(other.coefficients[other.degree()])
        exponent = self.degree() - other.degree()
        for i in range(0, exponent_max + 1, 1):
            list.append(0)
        newpoli = Polynomial(list)
        return newpoli.addition((self.substraction(other.multiplication(newpoli)).division(other)))
